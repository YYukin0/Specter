...
**P6.5 — `specdiff`：投机解码故障注入 + 差分调试器（头条交付物，~2.5 单元）— Parts 1–4 DONE + Part 5 部分**（a143f9c + b5d2ae9 + 本次）
...
- `results/p6_5_batch_invariance.json`（Part 5）—— batch-invariance 分类基线：无 mutant 时 FakeModel 批量 == 单序列 max token delta 0（180 对），specdiff 干净路径全 `NO_DIVERGENCE`。真实 MPS/int4 上 arXiv:2607.17283 的 ~5.8e-3 逐 logit delta（batch-variant kernel reduction，非算法 bug）归 `BACKEND_NONDETERMINISM`——那个实测数字是 O2 增量，分类契约已写进结果文件。
...
- 剩余：2510.22876 BSP 签名复现（转 P6.4 真实模型，架构上不可表达）、Part 6 工程故事、O2 真实模型 CPU-fp32 oracle（含 batch-invariance 实测 delta）。
