...
**P6.8 —（支柱7，工程落地物证）Web demo：stdlib HTTP + SSE + 单文件 lab 页** —— 之前的"落地"只有 P6.3 的终端 dashboard，README 里没提、也没 HTTP 面。P6.8 把 `SpecServer` 包成一个零新依赖的服务器 + 一个能当纯静态站的 research-lab 风页面。`src/serve_http.py`：`http.server` + `json` + `threading`，`GET /` 发 `docs/site/index.html`、`/health`、`/sample`（回放 JSON）；`POST /generate` = SSE 流，事件 `start` → `round`×N → `done` →（可选）`compare_done`，每轮带 `{index, mode, round_gamma, rolling_alpha, realignment_overhead, n_active, n_queued, emitted, wall_ms, tok_per_s, breaker_reason, texts}`（和终端 demo 打的同一套遥测）。单 Mac GPU → 用非阻塞 `threading.Lock` 串行化生成，并发 POST 返回 429。`--fake` 用 `make_fake_pair` + `LengthOnlyCache`（无下载，供 CI）；`--capture PATH` 真跑一次（Qwen2.5 0.5B/1.5B，`model_loader` + `spec_kv._new_cache`）把 SSE 时间线 dump 成回放 JSON（剔掉页面不读的 `tps_series` / `final_texts` 以缩小体积）。`docs/site/index.html`：单文件，近黑底 + 衬线大标题 + 小型大写节标签 + 一个静默绿 + 角括号卡片；五节 THE SYSTEM（build 表当卡片）/ NUMBERS（诚实 headline）/ NOTES（8 篇 note 当 finding-first 摘要卡，链到 GitHub 上的 .md）/ LAB / PITFALLS 链接。LAB：prompt textarea + 三 toggle（speculation / adaptive-γ / circuit-breaker）+ max-tokens 滑杆 + Run / Run A-B / Load 4-prompt batch；`fetch()` 读 SSE（`EventSource` 不能 POST）；实时遥测网格 + tok/s sparkline（canvas）+ 逐序列流式文本 + breaker 行；启动 `fetch('/health')` 失败 → banner「backend offline」+ 回放 `sample_run.json`（先试 `/sample` 再试相对路径，所以 GitHub Pages 直接能跑），回放按总时长 ~12s 配速。`docs/site/sample_run.json` = 提交进仓库的真实 4-prompt batch（demo_batch + compare，max_tokens=128）：spec agg 24.6 tok/s vs baseline 22.8 = **1.08×**，mean accept 2.5，~188 KB、174 事件。和 README「0.93–1.0× / parity」一致（单条 code prompt 能到 1.3×，聚合回到平手——没有 cherry-pick）。测试 `tests/test_serve_http.py`（9 hermetic：起真 `ThreadingHTTPServer` 于端口 0 跑 FakeModel，断言 `/health`、`GET /` 发页、SSE 线格式（start/round/done 字段、compare 加 baseline+speedup+baseline 无 spec 轮）、max_tokens 双端夹到 8..160、demo_batch 跑满 4 prompt、并发 429、未知路由 404）。pytest 210 → **219**。README 加「Demo」节（`python -m src.serve_http` 一条命令 + `curl -N -XPOST` 示例 + 终端版 `python -m demo.live --compare`）。坑25（HTTP/1.1 keep-alive + 无 Content-Length 的 SSE 流把客户端挂死；修 = `Connection: close` + `self.close_connection = True`，用 socket EOF 当流结束信号）见 §9.2。write-up：暂不单独成篇（当 README「Demo」节 + 站点本身），若之后要第 9 篇 note 可写「把研究代码包成能演示的东西：SSE、单 GPU 串行、静态回放」。
